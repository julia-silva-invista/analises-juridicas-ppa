...
from utils import _retry, _gerar_docx, _responder_pergunta_generica, _get_clients_proc, _barra_progresso
...
def proc_analisar(pdf_files, pdf_relacionados, instrucoes: str, usar_gemini_pro: bool = False, versao_resumida: bool = False):
    ...
    t_inicio = time.time()
    cache = None  # inicializado aqui para estar disponível em ambos os modos

    try:
        # Modo direto (1 chamada) para documentos pequenos
        if total_paginas <= MODO_DIRETO_MAX_PROC:
            log.append(f"\nModo direto ({total_paginas} pag. — 1 chamada ao {model_cons})")
            yield "\n".join(log), "", ""
            ...
            # Modo chunked com inline extraction + 2 chaves paralelas
            log.append(f"\nExtraindo {n} chunk(s) em paralelo (2 chaves · inline)...")
            ...
            def _worker_proc(idx):
                ...
                cli = client1 if idx % 2 == 0 else client2
                return _proc_extrair_chunk((idx, cp, offset, total_pg, n, cli))
