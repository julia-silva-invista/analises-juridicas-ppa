...
12. Instrução adicional da analista: quando o prompt trouxer a seção "INSTRUÇÃO ADICIONAL DA ANALISTA (PRIORITÁRIA)", ela tem precedência sobre a estrutura fixa deste template. Atenda ao pedido por inteiro, além de tudo o que o template já exige. Se o que foi pedido não couber em nenhum dos itens previstos, ACRESCENTE um item novo ao fim da subseção daquele processo, com título próprio no mesmo padrão dos demais bullets — não esprema a informação num item que não é dela, nem a omita por não haver item previsto. A instrução não autoriza inventar, estimar ou presumir: se o material não trouxer o que foi pedido, registre expressamente que não consta, com a mesma exigência de referência dos demais itens.
